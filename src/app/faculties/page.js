...
export default function FacultiesPage() {
    ...
    useEffect(() => {
        ...
        async function loadFaculties() {
            try {
                ...
                console.error('Failed to JIT load faculties', e);
            }
        }
        loadFaculties();
    ...
    const handleDeleteFaculty = async (id) => {
        try {
            await apiClient.delete(`/timetable/faculties/${id}`);
            dispatch({ type: ACTION_TYPES.DELETE_FACULTY, payload: id });
            setDeleteConfirm(null);
            addToast({ type: 'success', title: 'Faculty Deleted', message: 'The faculty was successfully deleted.' });
        } catch (e) {
            console.error(e);
            addToast({ type: 'error', title: 'Deletion Failed', message: e.message || 'An error occurred while deleting the faculty.' });
        }
    };
    ...
    const handleSaveDept = async () => {
        ...
        try {
            if (editingDept) {
                await apiClient.put(`/timetable/departments/${editingDept.id}`, { name: deptName.trim(), faculty_id: deptFacultyId });
                dispatch({ type: ACTION_TYPES.UPDATE_DEPARTMENT, payload: { id: editingDept.id, name: deptName.trim(), facultyId: deptFacultyId } });
            } else {
                const res = await apiClient.post('/timetable/departments', { name: deptName.trim(), faculty_id: deptFacultyId });
                dispatch({ type: ACTION_TYPES.ADD_DEPARTMENT, payload: { id: res.id, name: res.name, facultyId: res.faculty_id } });
            }
    ...
    const handleDeleteDept = async (id) => {
        try {
            await apiClient.delete(`/timetable/departments/${id}`);
            dispatch({ type: ACTION_TYPES.DELETE_DEPARTMENT, payload: id });
            setDeleteConfirm(null);
            addToast({ type: 'success', title: 'Department Deleted', message: 'The department was successfully deleted.' });
        } catch (e) {
            console.error(e);
            addToast({ type: 'error', title: 'Deletion Failed', message: e.message || 'An error occurred while deleting the department.' });
        }
    };
